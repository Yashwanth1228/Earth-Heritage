...
/**
 * Section 1 — Editorial Introduction for /managed-farmland
 * 
 * Composition:
 * Eyebrow: MANAGED FARMLAND
 * Heading: Land ownership, with professional care for the farm.
 * Supporting Copy: Managed farmland brings land ownership and ongoing farm management together...
 * Large Editorial Farmland Image: Dedicated high-resolution aerial and ground estate photo.
 */
export default function ManagedFarmlandIntro() {
  const { eyebrow, headingPart1, headingPart2, description } = managedFarmlandData.intro;

  return (
    <section
      id="hero"
      className="relative bg-[#FAF6F0] text-[#111613] pt-24 sm:pt-28 lg:pt-32 pb-14 sm:pb-20 overflow-hidden border-b border-[#DCCDB7]"
      aria-label="Managed Farmland Overview Introduction"
    >
      ...
      <Container size="default" className="relative z-10">
        ...
        <MotionReveal delay={0.35} className="mt-12 sm:mt-16 lg:mt-20 max-w-6xl mx-auto">
          <div className="relative w-full aspect-[16/10] sm:aspect-[16/9] lg:aspect-[21/9] rounded-2xl sm:rounded-3xl overflow-hidden border border-[#D5C09D] shadow-[0_20px_50px_rgba(17,22,19,0.09)] bg-[#E4D1B5]">
            <Image
              src={managedFarmlandImages.intro.src}
              alt={managedFarmlandImages.intro.alt}
              fill
              priority
