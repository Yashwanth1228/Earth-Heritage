...
/**
 * Section 9 — Frequently Asked Questions (FAQ) for /managed-farmland
 * 
 * 8 verified questions with accessible editorial accordion,
 * smooth animation transitions, and direct integration with enquiry modal.
 */
export default function ManagedFarmlandFaq() {
  const { openEnquiryModal } = useEnquiry();
  const { eyebrow, heading, description, items } = managedFarmlandData.faq;
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="border-t border-[#D5C09D]">
            {items.map((item, idx) => {
              ...
              return (
                <MotionReveal key={item.id} delay={0.06 + idx * 0.04}>
                  <div className="border-b border-[#D5C09D] transition-colors duration-200">
                    <button
                      ...
                      className="w-full py-5 sm:py-7 flex items-start justify-between gap-6 text-left group cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1E460B] focus-visible:ring-offset-2 rounded-sm"
                    >
                      <div className="flex items-baseline gap-4 sm:gap-6 flex-1">
                        ...
                        {/* Question */}
                        <span className="font-sans text-base sm:text-xl font-medium tracking-tight text-[#111613] group-hover:text-[#1E460B] transition-colors duration-200">
                          {item.question}
                        </span>
                      </div>
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        ...
                        >
                          <div className="pb-7 sm:pb-9 pl-8 sm:pl-12 pr-4 sm:pr-8 text-[#38423A] font-sans text-base sm:text-lg leading-relaxed space-y-4">
                            <p>{item.answer}</p>
                            {item.id === 'faq-8' && (
                              <div className="pt-2">
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    openEnquiryModal('Managed Farmland', e.currentTarget);
                                  }}
