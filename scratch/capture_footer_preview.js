...
const TEMP_USER_DATA = path.join(os.tmpdir(), 'chrome_cdp_footer_' + Date.now());
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < 30; i++) {
      await sleep(300);
      try {
        const targets = await httpGet('http://127.0.0.1:9277/json/list');
    ...
    const outPath = path.join(__dirname, 'footer_preview.png');
