...
export function getProjectsCollectionSchema(projectsList = []) {
  ...
  // Only attach item list if confirmed projects exist with valid slugs and names (excluding demo projects)
 ...
 * - Returns null if project or required fields (name, slug) are missing, or if project is a demo
 */
...
/**
 * Generate CollectionPage JSON-LD Schema for /events
 * 
 * Strict Schema.org CollectionPage standard:
 * - Connected to main WebSite entity via isPartOf
 * - References verified event item URLs
 */
export function getEventsCollectionSchema(eventsList = []) {
  ...
  const schema = {
    ...
    '@id': `${pageUrl}/#webpage`,
    url: pageUrl,
    name: 'Events & Farm Gatherings | Earth Heritage',
    description:
      'Scheduled community farm visits, seasonal harvest celebrations, agroforestry workshops, and landowner assemblies organized by Earth Heritage.',
    isPartOf: {
      '@id': `${siteConfig.url}/#website`
    }
  };
  ...
  if (validEvents.length > 0) {
    schema.mainEntity = {
      ...
      itemListElement: validEvents.map((e, index) => ({
        '@type': 'ListItem',
        position: index + 1,
        name: e.title,
        url: `${siteConfig.url}/events#${e.slug.trim()}`
      }))
    };
  }
...
/**
 * Generate Event JSON-LD Schema for verified event detail (/events/[slug])
 * 
 * Strict Schema.org Event standard:
 * - Only includes verified fields present in event data
 * - Omits missing attributes cleanly
 */
export function getEventDetailSchema(event) {
  if (!event || typeof event.title !== 'string' || !event.title.trim() || !event.slug) {
    return null;
  }

  const eventUrl = `${siteConfig.url}/events/${event.slug.trim()}`;
  ...
  const descriptionText = event.overview || event.tagline;
  if (typeof descriptionText === 'string' && descriptionText.trim().length > 0) {
    schema.description = descriptionText.trim();
  }
  ...
  if (
    event.coverImage &&
    typeof event.coverImage.src === 'string' &&
    event.coverImage.src.trim().length > 0
  ) {
    const imgSrc = event.coverImage.src.startsWith('http')
      ? event.coverImage.src
      : `${siteConfig.url}${event.coverImage.src.startsWith('/') ? '' : '/'}${event.coverImage.src}`;
    schema.image = imgSrc;
  }

  if (typeof event.location === 'string' && event.location.trim().length > 0) {
    schema.location = {
      '@type': 'Place',
      name: event.location.trim(),
      address: {
        '@type': 'PostalAddress',
        addressLocality: 'Bengaluru Region',
        addressCountry: 'IN'
      }
    };
  }
